...
export function AnimatePanel({ layerId }: { layerId: string }) {
  ...
  return (
    <div tabIndex={0} className="flex-1 overflow-y-auto min-h-0 outline-none focus:outline-none">
      {/* Duration control */}
      <div className="px-3 py-2.5 border-b border-hairline bg-surface-1">
        <div className="flex items-center gap-2 mb-2">
          ...
          <input
            ...
            className="ml-auto w-[52px] px-1.5 py-0.5 text-[11px] text-slate-200 bg-[#11151f] border border-[#1c2230] rounded text-center focus:outline-none focus:border-accent-dim"
          />
        ...
        <input
          ...
          className="w-full h-1 bg-[#1c2230] rounded appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-accent [&::-webkit-slider-thumb]:cursor-pointer"
        />
      ...
      {/* At The Start toggle */}
      <div className="px-3 py-2 border-b border-hairline bg-surface-1">
      ...
      {/* Selection info */}
      {targetIds.length > 1 && (
        <div className="px-3 py-1.5 border-b border-hairline bg-[#0d1a2e]">
        ...
        <div key={category} className="border-b border-[#13182370]">
          ...
          <div className="grid grid-cols-2 gap-1.5 px-3 pb-3">
            {grouped[category].map((preset) => (
              ...
              >
                <span className="text-[11.5px] font-medium text-slate-200 group-hover:text-accent-hover leading-tight">
                  {preset.name}
                </span>
