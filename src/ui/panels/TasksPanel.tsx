...
function StatusIcon({ status }: { status: TaskStatus }) {
  if (status === 'running') return <Loader2 size={12} className="text-accent animate-spin flex-shrink-0 mt-0.5" />;
  ...
  return (
    <aside className="flex-shrink-0 h-full flex flex-col bg-surface-sunken border-l border-hairline" style={{ width: '20%', minWidth: 260 }}>
      ...
      <div ref={scrollRef} className="flex-1 min-h-0 overflow-y-auto px-3 py-3 space-y-2">
        ...
        {items.map((it) => (
          <div key={it.id} className="rounded-md bg-[#0e1726] border border-hairline px-2.5 py-2">
                ...
                {typeof it.progress === 'number' && (
                  <div className="mt-1.5 flex items-center gap-1.5">
                    <div className="flex-1 h-1 rounded bg-surface-3 overflow-hidden">
                      <div className="h-full bg-accent transition-all" style={{ width: `${Math.max(2, Math.min(100, it.progress))}%` }} />
                    </div>
