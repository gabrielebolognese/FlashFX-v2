...
// progress on first run, then "Transcribing clip N of M…", with cancel. The editor stays interactive
// underneath. Also surfaces a dismissible error (e.g. no speech, or model failed to load offline).
export function AutoCaptionProgress() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[90] w-72 rounded-lg border border-[#26364f] bg-[#0d1524] shadow-2xl shadow-black/50 p-3 select-none">
      {error ? (
        <div className="flex items-start gap-2">
          ...
          <button title="Dismiss" onClick={dismissError} className="p-1 rounded text-slate-500 hover:text-slate-200 hover:bg-white/5"><X size={12} /></button>
        </div>
      ) : (
        <>
          <div className="flex items-center gap-2">
            ...
            <button title="Cancel" onClick={cancel} className="p-1 rounded text-slate-500 hover:text-slate-200 hover:bg-white/5"><X size={12} /></button>
          </div>
          {download && (
