...
| B5b | Frame-mix frame blending (WebCodecs/GPU two-frame cross-dissolve) | ▶ **next** | medium |
...
## B5b — Frame-mix frame blending ▶ NEXT
**Delivers:** **frame-mix** (Frame Mix / "frame blending") for retimed/slowed video — cross-dissolve the two adjacent source frames instead of hard-cutting to one, so slow-mo doesn't stutter. (Optical-flow "buttery" slow-mo is the separate, heavier B6.) **Categories:** 2, 1. **Depends on:** B5a (✅) — the retime model now floors to one source frame; this surfaces the *fractional* source position. **Perf:** medium — one extra texture bind + a shader `mix`; also update the scheduler's constant-velocity lookahead for non-linear ramps. **Browser-gated** (WebCodecs/WebGPU): not node-harnessable. **Likely files:** `core/interpolation.ts` (surface fractional source frame), `core/types.ts` (`ResolvedVideo` → `sourceFrameA`/`sourceFrameB`/`mix` + a `frameBlend` flag), `engine/video/frameScheduler.ts` + `videoTextureCache.ts` (hold/bind two textures), `engine/renderer.ts` (two-texture cross-dissolve uniform), Inspector toggle. **Verify:** the fractional-source math is node-harnessable (extend `verify:time-remap`); the two-texture blend is browser-verified.
