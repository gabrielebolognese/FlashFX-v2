...
| B6b | Optical-flow GPU estimator + warp pre-pass (WebGPU, browser-gated) | ▶ **next** | **heavy** |
...
## B6b — Optical-flow GPU estimator + warp pre-pass ▶ NEXT
**Delivers:** the actual **motion-estimated slow-mo** — a WGSL optical-flow estimator (Lucas-Kanade/Horn-Schunck as ping-pong fragment passes between the two decoded source frames) + a **warp pre-pass** (precomp-pattern: read frameA/frameB along ±`t`·flow → one warped texture the normal video draw then samples, no shared-path change) + a flow-field `LruCache`; then **pixel motion blur on footage** as a flow-driven blur pass. **Categories:** 2, 1. **Depends on:** B6a (✅: mode, pair plumbing, cache-key), B5b (✅: two frames already decoded via `:fb`). **Perf:** **HEAVY** — iterative passes per frame pair + flow cache + quality/LOD gating. **⚠ Browser-gated & NOT node-verifiable** (WGSL compiles/run only in a browser; can't even be confirmed to compile from here) — build with a browser available to drive it. Contain via lazy, try/catch pipeline init → **fall back to frame-mix on any failure** (opt-in, off by default), so existing video is never at risk. **Likely files:** `engine/video/opticalFlow.ts` (WGSL + a guarded pipeline/texture-pool helper), `engine/renderer.ts` (warp pre-pass + flow-driven blur pass), a flow-field cache via `engine/cache/lruCache.ts`. **Verify:** extend `verify:optical-flow` for any added pure math; the estimator/warp/blur shaders are browser-verified.
