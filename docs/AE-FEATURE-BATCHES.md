...
| B8d | In-shape Repeater operator (per-copy transform accumulation) | ▶ **next** | medium |
| B8e | Gradient strokes on pen paths (CPU-bake or per-pixel shader, browser-gated) | ⬜ | medium |
...
### B8d — In-shape Repeater operator ▶ NEXT
...
### B8e — Gradient strokes on pen paths
**Delivers:** the multi-layer gradient stroke (`ShapeLayer.strokeMaterialConfig` → `ResolvedShape.stroke`, already resolved for polygons but ignored by the path pipeline) on pen-path shapes; SDF shapes already render it. **Approach (from the audit):** the gradient COLOR math (`sampleFill`/`gradientT`, renderer.ts) is SDF-independent — either (A) **CPU-bake** the resolved stroke gradient per stroke-vertex into the existing `[…,r,g,b,a]` slots (pure/harnessable, Gouraud-approximate, no shader change) or (B) port `sampleFill` into `PATH_SHADER` + add a uv/box vertex attribute for per-pixel exactness (**browser-gated** — PATH_VERTEX_STRIDE/attributes/WGSL changes). **Perf:** medium. **Verify:** a pure CPU gradient sampler is harnessable; per-pixel path is browser-verified.
